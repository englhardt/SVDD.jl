...
@testset "SVDDnegEps" begin
    ...
    @testset "with outliers and epsilon" begin
        C1 = C2 = 0.5
        gamma = 0.5
        eps = 0.1
        init_strategy = SVDD.FixedParameterInitialization(MLKernels.GaussianKernel(gamma), (C1, C2))
        SVDD.initialize!(svdd_neg, init_strategy)
        SVDD.set_eps!(svdd_neg, eps)
        SVDD.fit!(svdd_neg, TEST_SOLVER)
        predictions = SVDD.predict(svdd_neg, dummy_data)
        @test sum(predictions .> 0) < 8
    ...
    @testset "params" begin
        ...
        @test all(map(x->SVDD.is_valid_param_value(svdd_neg, Val{:eps}, x), [0.0, 0.1, 1.0, 5.0]))
        ...
        @test !any(map(x->SVDD.is_valid_param_value(svdd_neg, Val{:eps}, x), [-0.1]))
    end
end
